...
#define CONTROL 7 //control pushbutton in digital pin 7
...
double fLog = 0.01;      // logarithm of normalized frequency
...
void setup()
{
  ...
  pinMode(CONTROL, INPUT);    // sets the digital pin as output
...
void loop()
{
  ...
  fLog = log(normf) / log(gap); // allows us to take the log (base gap) and the result 
  ilogf = round(fLog);           // is the number of notes above the lowest, once we round it.
  f = minfreq * pow(gap,ilogf);  // we better "unlog" it.

  //int ctrl = digitalRead(CONTROL);
